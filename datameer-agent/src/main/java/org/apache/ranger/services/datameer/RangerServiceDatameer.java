 ...
 * under the License.
 */
package org.apache.ranger.services.datameer;
...
import org.apache.ranger.authorization.hadoop.RangerDatameerAuthorizer;
...
import org.apache.ranger.services.datameer.client.DatameerResourceMgr;

public class RangerServiceDatameer extends RangerBaseService {

	private static final Log LOG = LogFactory.getLog(RangerServiceDatameer.class);
	...
	public Map<String, Object> validateConfig() throws Exception {
		...
		if (configs != null) {
			try {
				ret = DatameerResourceMgr.connectionTest(serviceName, configs);
				// ret.putAll(DatameerHdfsResourceMgr.connectionTest(serviceName, configs));
	...
	public List<String> lookupResource(ResourceLookupContext context) throws Exception {
		...
		if (context != null) {
			try {
				ret = DatameerResourceMgr.getDatameerResources(serviceName, serviceType, configs, context);
			} catch (Exception e) {
	...
	public List<RangerPolicy> getDefaultRangerPolicies() throws Exception {
		...
		String pathResourceName = RangerDatameerAuthorizer.KEY_RESOURCE_PATH;
		...
		for (RangerPolicy defaultPolicy : ret) {
			...
			if (pathPolicyResource != null) {
				...
				if (pathResourceDef != null) {
					String pathSeparator = pathResourceDef.getMatcherOptions()
							.get(RangerPathResourceMatcher.OPTION_PATH_SEPARATOR);
		...
		try {
			...
			for (List<RangerServiceDef.RangerResourceDef> aHierarchy : serviceDefHelper
					.getResourceHierarchies(RangerPolicy.POLICY_TYPE_ACCESS)) {
	...
	private RangerPolicy getPolicyForKMSAudit(List<RangerServiceDef.RangerResourceDef> resourceHierarchy)
			throws Exception {
	...
	private Map<String, RangerPolicy.RangerPolicyResource> createKMSAuditResource(
			List<RangerServiceDef.RangerResourceDef> resourceHierarchy) throws Exception {
		if (LOG.isDebugEnabled()) {
			LOG.debug("==> RangerServiceHdfs.createKMSAuditResource()");
		}
		Map<String, RangerPolicy.RangerPolicyResource> resourceMap = super.createDefaultPolicyResource(
				resourceHierarchy);

		RangerPolicy.RangerPolicyResource pathResource = resourceMap.get(RangerDatameerAuthorizer.KEY_RESOURCE_PATH);

		if (pathResource != null) {
			pathResource.setValue(AUDITTOHDFS_KMS_PATH);
		} else {
			LOG.error("Internal error: Could not find RangerPolicyResource corresponding to "
					+ RangerDatameerAuthorizer.KEY_RESOURCE_PATH + " in default policy-resource");
		}
